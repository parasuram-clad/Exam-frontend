...
const MyProgress = () => {
  ...
  // Fetch dashboard data using the current context
  ...
  const test_wise_progress = dashboardData?.test_wise_progress;
  const dailyPerf = dashboardData?.daily_performance;
  ...
  const testWiseData = (test_wise_progress?.parts || test_wise_progress?.subjects || []).map((p: any) => ({
    name: p.part || p.part_name || p.subject_name || p.name || "Item",
    score: Math.round(p.progress ?? p.average_score ?? p.accuracy ?? p.score ?? 0),
    color: BAR_COLOR,
  }));
    ...
    <DashboardLayout>
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {features?.subject_wise_progress && (
            <motion.div variants={itemVariants}>
