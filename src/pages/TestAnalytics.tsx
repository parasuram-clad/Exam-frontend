...
const TestAnalytics = () => {
    ...
    const completedDate = new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });
        ...
        <DashboardLayout
            ...
            rightSidebar={
                <div className="space-y-6">
                    ...
                    {/* Your Rank Card */}
                    <div className="bg-[#FFFAFA] rounded-2xl p-5 sm:p-6 flex items-center justify-between shadow-sm overflow-hidden relative">
                        <div className="relative z-10">
                            <p className="text-xs sm:text-sm font-medium text-foreground">Your Rank</p>
                            <p className="text-4xl sm:text-5xl lg:text-6xl font-semibold text-foreground leading-tight">{data.yourRank}</p>
                            <p className="text-[10px] sm:text-xs text-muted-foreground mt-1">Keep going!</p>
                        </div>
                        <img src={trophyRank} alt="Trophy" className="w-20 h-20 sm:w-28 sm:h-28 lg:w-32 lg:h-32 object-contain relative z-10" />
                        <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full -mr-16 -mt-16 sm:hidden" />
                    </div>

                    {/* Leaderboard */}
                    <LeaderboardWidget data={data.leaderboard} />
                </div>
            }
        >
            ...
            <div
                ...
                style={{ background: 'linear-gradient(to bottom right, #FAFFFC 0%,  #FAFFF1 100%)' }}
            >
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-card rounded-2xl border border-border shadow-sm p-4 sm:p-6 lg:p-10">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                                    ...
                                    <Button variant="outline" className="rounded-xl gap-2 w-full sm:w-auto px-6 h-11 hover:bg-[#f3f4f6] transition-colors">
                                        Check All Answers <ArrowRight className="w-4 h-4" />
                                    </Button>
                                </div>
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 text-left min-w-0">
                                                        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                                                            <h4 className="font-semibold text-foreground text-sm sm:text-md truncate">{unit.name}</h4>
                                                            <span className={cn("text-[8px] sm:text-[9px] px-2 sm:px-3 py-0.5 sm:py-1 rounded-full font-semibold uppercase tracking-wider", unit.badgeColor)}>
                                                                {unit.badge}
                                                            </span>
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
                                                        <div className="relative">
                                                            ...
                                                            <div className="absolute inset-0 flex items-center justify-center">
                                                                <span className="text-[10px] sm:text-xs font-semibold text-foreground">{unit.score}</span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="ml-0 ">
                                                            <div className="rounded-2xl border border-blue-100 bg-blue-50/20 overflow-hidden shadow-inner overflow-x-auto">
                                                                <div className="min-w-[400px]">
                                                                    <div className="grid grid-cols-[1fr_80px_80px] sm:grid-cols-[1fr_100px_100px] bg-blue-50/40 px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-xs font-semibold text-muted-foreground uppercase tracking-widest">
                                                                    ...
                                                                    {unit.topics.map((topic, idx) => (
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span className="text-center">
                                                                                <button className="text-xs sm:text-sm font-semibold text-primary hover:underline underline-offset-4 decoration-2">Review</button>
                                                                            </span>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            ...
                                                            <div className="flex justify-start mt-4">
                                                                <Button className="bg-foreground hover:bg-foreground/90 text-background rounded-xl gap-2 font-semibold h-10 sm:h-11 px-6 text-sm">
                                ...
                                <div className="mt-8 flex items-start gap-4 bg-amber-50 rounded-2xl p-5 border border-amber-200/50 shadow-sm">
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-amber-900 text-sm">Actionable Insight</h4>
