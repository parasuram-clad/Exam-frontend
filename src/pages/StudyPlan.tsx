...
const StudyPlan = () => {
  ...
  const { data: topicTimings = [] } = useQuery({
    ...
    queryFn: () => studyService.getUserTopicTimings(), // No argument passed, will fetch all for current user
    enabled: !!user?.id,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const testSubmitMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      if (planType === 'SUBJECT') {
        if (testType === 'MONTHLY') {
          navigate(`/test-series/subject-monthly/test/${testId}/analytics?month=${month}${planParam}`);
        } else {
          navigate(`/test-series/subject-weekly/test/${testId}/analytics?week=${week}${planParam}`);
        }
      } else {
        if (testType === 'MONTHLY') {
          navigate(`/test-series/monthly/test/${testId}/analytics?month=${month}${planParam}`);
        } else {
          navigate(`/test-series/weekly/test/${testId}/analytics?week=${week}${planParam}`);
        }
      }
    },
    onError: (error: any) => {
  ...
  const overallProgress = useMemo(() => {
    ...
    topicTimings.forEach((t: TopicTiming) => {
      ...
      if (!t.end_time && t.start_time && !statusMap[t.syllabus_id]) {
        const startTimeStr = t.start_time.endsWith('Z') ? t.start_time : `${t.start_time}Z`;
        const start = new Date(startTimeStr);
        if (!isNaN(start.getTime())) {
          m += Math.max(0, Math.round((now.getTime() - start.getTime()) / (1000 * 60)));
        }
      }
  ...
  const sortedAvailableSubjects = useMemo(() => {
    if (roadmapData?.access?.subject_plans && Array.isArray(roadmapData.access.subject_plans)) {
      const subjectsFromMetadata = roadmapData.access.subject_plans.map((p: any) => p.subject_name).filter(Boolean).sort() as string[];
      if (subjectsFromMetadata.length > 0) return subjectsFromMetadata;
    }
    if (roadmapData?.plan && Array.isArray(roadmapData.plan)) {
      const subjectsFromPlanItems = Array.from(new Set(roadmapData.plan.filter(p => p.plan_type === 'SUBJECT' && (p.subject_name || p.label)).map(p => (p.subject_name || p.label)!))).sort();
      if (subjectsFromPlanItems.length > 0) return subjectsFromPlanItems;
    }
    const overallPlan = roadmapData?.plan?.find(p => p.plan_type === 'OVERALL');
    if (overallPlan?.days) {
      const subjectsInOverall = new Set<string>();
      overallPlan.days.forEach(d => { d.items.forEach((item: any) => { if (item.type === 'TOPIC' && item.subject) subjectsInOverall.add(item.subject); }); });
      if (subjectsInOverall.size > 0) return Array.from(subjectsInOverall).sort();
    }
    return [];
  }, [roadmapData]);
  ...
  const currentStudyTopics = (hasDynamicData ? (dynamicDayWisePlans[activeDay] || []) : (dayCycleRotation[activeDay] || dayCycleRotation[activeDay % 7] || []))
    .filter(t => {
      ...
      if (selectedSubject) return t.title === selectedSubject || t.type === 'TEST' || t.type === 'REVISION';
      return true;
    });
  ...
  const handleDayClick = (day: DayCycleItem) => {
    ...
    else if (isPrevAssessmentMissing) toast.warning(`Please complete Assessment on Day ${previousAssessmentDay}.`);
    setActiveDay(day.day);
    const dayTopics = dynamicDayWisePlans[day.day] || [];
    if (dayTopics.length > 0 && user?.id) dayTopics.forEach(topic => { topic.subtopics.forEach(st => prefetchTopic(queryClient, st.id, user.id)); });
  };
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      ...
      await studyService.generateStudyPlan({ user_id: user!.id, exam_type: setupData.examType, sub_division: setupData.subDivision.join(", "), year: parseInt(setupData.targetYear), learner_type: setupData.learnerType, daily_study_hours: dailyHours, language: setupData.medium === 'tamil' ? 'Tamil' : 'English' });
      await Promise.all([queryClient.invalidateQueries({ queryKey: ['user-me'] }), queryClient.invalidateQueries({ queryKey: ['study-plans', user!.id] }), queryClient.invalidateQueries({ queryKey: ['roadmap', user!.id] })]);
      setActiveDay(1);
      toast.success("Study plan generated successfully!");
      setIsSetupModalOpen(false); // Close only after success
    } catch {
  ...
  const handleSubtopicClick = async (topicId: string, subtopicId: string) => {
      ...
      if (subtopic?.isTest) {
        ...
        try {
          ...
          if (subtopic.status === 'completed') {
            const pId = currentPlan?.plan_id;
            const planParam = pId ? `&plan_id=${pId}` : '';
            if (testType === 'MONTHLY') navigate(`/test-series/${planType === 'SUBJECT' ? 'subject-monthly' : 'monthly'}/test/0/analytics?month=${monthNo}${planParam}`);
            else navigate(`/test-series/${planType === 'SUBJECT' ? 'subject-weekly' : 'weekly'}/test/0/analytics?week=${weekNo}${planParam}`);
          ...
          if (planType === 'SUBJECT') {
            ...
            response = await (testType === 'MONTHLY' ? studyService.getSubjectMonthlyTestQuestions(sId, monthNo, pId) : studyService.getSubjectWeeklyTestQuestions(sId, weekNo, pId));
          } else {
            response = await (testType === 'MONTHLY' ? studyService.getMonthlyTestQuestions(user!.id, monthNo, pId) : studyService.getWeeklyTestQuestions(user!.id, weekNo, pId));
          }
          const transformed: TestQuestion[] = response.questions.map((q: any) => ({ id: q.mcq_id, question: q.question, options: Object.values(q.options), correctAnswer: 0, category: testType === 'MONTHLY' ? "Monthly" : "Weekly", difficulty: "Medium" }));
          setWeeklyTestQuestions(transformed);
    ...
    if (!isNaN(subIdNum)) {
      const plan = userPlans.find(p => p.id === subIdNum || (p.syllabus_id === subIdNum && p.plan_status !== 'COMPLETED'));
      console.log(`Checking subtopic click: id=${subIdNum}, foundPlanStatus=${plan?.plan_status}`);
      if (plan && (plan.plan_status === 'ACTIVE' || !plan.plan_status)) {
        try {
          setIsFetchingQuestions(true);
    ...
    const planRec = userPlans.find((p: any) => p.syllabus_id === subIdNum || p.id === subIdNum);
    const pId = planRec?.plan_id || currentPlan?.plan_id;
    const query = pId ? `?plan_id=${pId}` : '';

    setIsDialogOpen(false);
    navigate(`/study-plan/topic/${topicId}/subtopic/${subtopicId}${query}`);
  };

  const bannerProgress = (viewMode === 'subject' && !selectedSubject) ? (userPlans.length === 0 ? 0 : Math.round((userPlans.filter(p => p.is_completed).length / userPlans.length) * 100)) : overallProgress;
  ...
  const initials = (user?.full_name || user?.username || "A").split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase();
  ...
  return (
    <DashboardLayout
      ...
      rightSidebar={() => (
        <StudyPlanRightSidebar
          user={user} avatarUrl={avatarUrl} initials={initials} onDateClick={(date) => { const dateStr = format(date, 'yyyy-MM-dd'); const dayMatch = currentPlan?.days?.find(d => d.date === dateStr); if (dayMatch) setActiveDay(dayMatch.day); }}
    ...
    >
      <div className="px-1 mb-2">
        <motion.div variants={itemVariants} initial="hidden" animate="visible" className="flex flex-col sm:flex-row sm:items-end gap-1">
          <div>
            <div className="flex items-center gap-2">{(selectedTopic || (selectedSubject && showPlanToggle) || (selectedSubject && !hasOverallPlanData)) && (<button onClick={() => { setSelectedTopic(null); setSelectedSubject(null); }} className="p-1 hover:bg-muted rounded-full mr-1"><ArrowLeft className="w-4 h-4" /></button>)}<h1 className="text-xl sm:text-2xl font-medium">{selectedSubject ? selectedSubject : (currentPlan?.label || "Study Plan")}</h1></div>
          ...
          {showPlanToggle && (<div className="flex bg-muted/30 p-1 rounded-xl sm:ml-auto"><button onClick={() => { setViewMode('overall'); setSelectedSubject(null); }} className={cn("px-4 py-2 text-xs font-medium rounded-lg", viewMode === 'overall' ? "bg-white shadow-sm" : "")}>Overall</button><button onClick={() => setViewMode('subject')} className={cn("px-4 py-2 text-xs font-medium rounded-lg", viewMode === 'subject' ? "bg-white shadow-sm" : "")}>Subject</button></div>)}
        </motion.div>
      </div>
        ...
        {(!roadmapData?.plan || roadmapData.plan.length === 0) ? (
          <motion.div variants={itemVariants} className="w-full bg-card rounded-2xl p-12 border border-dashed flex flex-col items-center justify-center text-center space-y-6">
            ...
            <Button
              ...
              className="rounded-full px-10 h-12 bg-[#1a2b4b]"
            >
              {isGenerating ? (
          ...
          <motion.section variants={itemVariants}><h2 className="text-lg font-medium mb-5">Select a Subject</h2><SubjectPlanView subjects={sortedAvailableSubjects || []} onSelectSubject={setSelectedSubject} userPlans={userPlans || []} roadmapData={roadmapData} topicTimings={topicTimings} /></motion.section>
        ) : (
          <>
      ...
      <StudyTopicDetailDialog
        ...
        topicTimings={topicTimings}
        user={user}
        onSubtopicClick={handleSubtopicClick}
        getSubjectIconFallback={getSubjectIconFallback}
      ...
      <WeeklyTestModal
        ...
        onSubmit={(answersRecord) => {
          ...
          const testType = currentTestType || "WEEKLY";
          const planType = currentPlan?.plan_type || "OVERALL";
          ...
          testSubmitMutation.mutate({
            ...
            planType: planType,
            [testType === 'MONTHLY'
              ? (planType === 'SUBJECT' ? 'subject_monthly_test_id' : 'monthly_test_id')
              : (planType === 'SUBJECT' ? 'subject_weekly_test_id' : 'weekly_test_id')
            ]: currentWeeklyTestId,
            answers,
